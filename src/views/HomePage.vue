...
<template>
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar>
        <ion-title>Home Ratatouille</ion-title>
      </ion-toolbar>
    </ion-header>
        ...
        <ion-accordion-group>
          <ion-accordion value="first">
            <ion-item slot="header" color="light">
              <ion-label class="ion-text-center">Completed Resep</ion-label>
            </ion-item>
...
const handleSubmit = async (
  ...
  if (!todo.title) {
    await showToast("Resep is required", "warning", warningOutline);
    return;
  }
  try {
    if (editingId.value) {
      await firestoreService.updateTodo(editingId.value, todo as Todo);
      await showToast("Resep updated successfully", "success", checkmarkCircle);
    } else {
      await firestoreService.addTodo(todo as Todo);
      await showToast("Resep added successfully", "success", checkmarkCircle);
    }
    loadTodos();
  } catch (error) {
...
const handleDelete = async (deleteTodo: Todo) => {
  try {
    ...
    await showToast("Resep deleted successfully", "success", checkmarkCircle);
    loadTodos();
  } catch (error) {
    await showToast("Failed to delete resep", "danger", closeCircle);
    console.error(error);
  }
};
...
const handleStatus = async (statusTodo: Todo) => {
  ...
  try {
    ...
    await showToast(
      `Resep marked as ${!statusTodo.status ? "completed" : "active"}`,
      "success",
      checkmarkCircle
    );
    loadTodos();
